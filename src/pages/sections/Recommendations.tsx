...
import recommendationsData from '../../helper/recommendations.json'
    ...
    <section id="recommendations" className="py-20 px-6 lg:px-12 bg-[#F0F0F6]">
      <div className="max-w-6xl mx-auto">
        <ScrollAnimation animation="fade-up">
          <h2 className="section-title text-center">{recommendationsData.title}</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          {recommendationsData.testimonials.map((testimonial, index) => (
            <ScrollAnimation
              key={index}
              animation="fade-up"
              delay={index * 100}
              ...
              <div className="card bg-white">
                <div className="flex gap-1 mb-4">
                  {[...Array(testimonial.rating)].map((_, i) => (
                    <img key={i} src={recommendationsData.starIcon} alt="star" className="w-4 h-4" />
                  ))}
                </div>
